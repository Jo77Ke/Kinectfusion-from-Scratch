...
void bilateralFilter(
    ...
    for (int v1 = 0; v1 < imageHeight; ++v1) {
        const auto* depthRow = depthMap.ptr<float>(v1);
        auto* denoisedRow = denoisedDepthMap.ptr<float>(v1);

        for (int u1 = 0; u1 < imageWidth; ++u1) {
            const Eigen::Vector2i pixel1(u1, v1);
            auto depth1 = depthRow[u1];
